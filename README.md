...
- **📐 多画幅・不裁切** —— 16:9 / 4:3 / 3:4 / 9:16 / 1:1 / 21:9 / 自定义，整个画面完整缩放进所选画幅（不够处留边，内容零丢失），横竖屏一次选好，适配抖音、视频号、B站、YouTube
...
拆模块的话：屏幕+人像 Canvas 实时合成管线、Canvas 美颜滤镜链、绿幕色键抠像、悬浮提词器（不入录制画面）、多画幅适配（整页缩放不裁切），每一块都能独立搬走（见上面技术要点表）。
